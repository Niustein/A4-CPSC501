...
//	maxLength = max(numInput,numImpulse);
	...
	// multiply n by itself to get nn
	numSquared *= 2;
	...
	for (i = 0; i < numInput; i++){
		inputPadded[i*2] = signalInput[i];
	}
	// Put audio data into new array impulsePadded
	for (i = 0; i < numImpulse; i++){
		impulsePadded[i*2] = impulseResponse[i];
	}
	
	cout << "start four1 algorithm" << endl;
		
	four1(inputPadded - 1, nHolder, 1);
	four1(impulsePadded - 1, nHolder, 1);
	...
	for(i=0; i < numResult; i++){
		...
		if((abs(oData[i * 2])) > maxRange){
			maxRange = abs(oData[i*2]);
		}
	}
