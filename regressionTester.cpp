...
bool checkEqual(double*, double*, int);
...
bool checkEqual(double baseData[], double FFTData[], int dLength){
	
	cout << "comparing equal" << endl;
	
	int i;
	bool equalFlag = true;
	
	for(i = 0; i < dLength; i++){
		if (baseData[i] != FFTData[i]){
			equalFlag = false;
			return equalFlag;
		}
	}
...
int main(int argc, char * const argv[]){
    ...
    AudioHeader baseConvolve(argv[1]);
    
    cout << "reading IR file: " << argv[2] << endl;
    AudioHeader FFTConvolve(argv[2]);
    
    cout << "iAudio header information:" << endl;
    printHeader(baseConvolve);
	cout << endl;
    
    cout << "irAudio header information:" << endl;
    printHeader(FFTConvolve);
    
    cout << "finished reading wave files" << endl;
		
	bool equalFlag = true;
    
    if(baseConvolve.subChunk2Size == FFTConvolve.subChunk2Size){
		cout << "base + fft have equal size" << endl;
		int dLength = (baseConvolve.subChunk2Size / 2);
		
		if(checkEqual(baseConvolve.dataA, FFTConvolve.dataA, dLength)){
			// Do nothing
		} else {
			equalFlag = false;
		}
